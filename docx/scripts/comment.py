...
# ---------------------------------------------------------
# Standardised mapping-comment builder (same fields as Excel)
# ---------------------------------------------------------

def build_mapping_comment(
    match_type: str,
    gac_id_list: list[str] | None = None,
    diff_detail: str = "",
    conclusion: str = "",
    no_match_reason: str = "",
) -> str:
    """Build a mapping comment string that mirrors the Excel ``差异描述`` column.

    Both Word comments and Excel rows should call this function so their
    content never diverges.

    Args:
        match_type: ``"一对一"`` / ``"一对多"`` / ``"多对一"`` / ``"无对应"``.
        gac_id_list: List of matched GAC FUNC IDs (empty for 无对应).
        diff_detail: The **same** ``diff_detail`` field used by the Excel
                     ``差异描述`` column.  This is the single source of truth.
        conclusion: ``"一致"`` / ``"部分差异"`` / ``"显著差异"`` etc.
        no_match_reason: Reason string when match_type is ``"无对应"``.

    Returns:
        Formatted comment string ready for ``add_comment()``.

    Example::

        text = build_mapping_comment(
            match_type="一对多",
            gac_id_list=["FUNC_0103", "FUNC_0105", "FUNC_0073"],
            diff_detail="核心信号VCU_BattSOCUndrWarn完全匹配...",
            conclusion="部分差异",
        )
        # → "[映射] 类型: 一对多; 目标: FUNC_0103, FUNC_0105, FUNC_0073\\n"
        #   "[部分差异] 核心信号VCU_BattSOCUndrWarn完全匹配..."
    """
    if match_type == "无对应":
        reason = no_match_reason or "GAC文档中无对应功能实现"
        return f"[无对应] 原因: {reason}"

    ids_str = ", ".join(gac_id_list) if gac_id_list else "无"
    header = f"[映射] 类型: {match_type}; 目标: {ids_str}"

    if not diff_detail and not conclusion:
        return header

    tag = conclusion or match_type
    body = diff_detail.strip() if diff_detail else "（无差异详情）"
    return f"{header}\n[{tag}] {body}"
